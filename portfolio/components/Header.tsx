  ...
  return (
    <>
      ...
      >
        ...
        <div className="fixed top-3 right-3 z-50 flex flex-row-reverse items-center gap-3">
          <button
            onClick={() => {
              if (menuOpen) {
                setMenuClosing(true);
                setMenuOpen(false);
                setTimeout(() => setMenuClosing(false), 700);
              } else {
                setMenuOpen(true);
              }
            }}
          ...
          >
            <div className="flex flex-col gap-[5px]">
              <span
                className={`block w-5 h-px bg-current transition-all duration-200 ${menuOpen || menuClosing ? "rotate-45 translate-y-[6px]" : ""}`}
              />
              <span
                className={`block w-5 h-px bg-current transition-all duration-200 ${menuOpen || menuClosing ? "opacity-0" : ""}`}
              />
              <span
                className={`block w-5 h-px bg-current transition-all duration-200 ${menuOpen || menuClosing ? "-rotate-45 -translate-y-[6px]" : ""}`}
              />
          ...
          {(menuOpen || menuClosing) && (
            <>
              <a
                ...
                title="GitHub"
                className={`text-white/50 hover:text-white transition-colors ${menuClosing ? "animate-menu-item-out" : "animate-menu-item"}`}
                style={{ animationDelay: menuClosing ? "0s" : "0.3s" }}
              >
              ...
              <a
                ...
                title="LinkedIn"
                className={`text-white/50 hover:text-white transition-colors ${menuClosing ? "animate-menu-item-out" : "animate-menu-item"}`}
                style={{ animationDelay: menuClosing ? "0.1s" : "0.2s" }}
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <a
                ...
                title="X (Twitter)"
                className={`text-white/50 hover:text-white transition-colors ${menuClosing ? "animate-menu-item-out" : "animate-menu-item"}`}
                style={{ animationDelay: menuClosing ? "0.2s" : "0.1s" }}
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <a
                ...
                className={`text-white/50 hover:text-white transition-colors ${menuClosing ? "animate-menu-item-out" : "animate-menu-item"}`}
                style={{ animationDelay: menuClosing ? "0.3s" : "0s" }}
              >
